...
pub struct BasicSettings {
    ...
    pub background_type: BGType,
}

fn default_bg_type() -> BGType {
    BGType::Image
}
...
pub fn create_new_config(
    ...
    // Find Games dir root
    let games_root = get_game_config_path(&app, "Dummy")
        .parent()
        .unwrap()
        .parent()
        .unwrap()
...
pub fn delete_game_config_folder(app: AppHandle, game_name: String) -> Result<(), String> {
    ...
    if game_dir.exists() {
        fs::remove_dir_all(game_dir).map_err(|e| format!("Failed to delete directory: {}", e))?;
    }
...
pub fn set_game_background(
    ...
    let mut config =
        load_game_config(app.clone(), game_name.clone()).unwrap_or(GameConfig::default());
    config.basic.background_type = bg_type;
    save_game_config(app.clone(), game_name, config)
...
pub fn set_game_icon(app: AppHandle, game_name: String, file_path: String) -> Result<(), String> {
    ...
    fs::copy(&source_path, &target_path).map_err(|e| format!("Failed to copy icon: {}", e))?;

    Ok(())
}
#[tauri::command]
pub async fn update_game_background(
    app: AppHandle,
    game_name: String,
    game_preset: String,
    bg_type: BGType,
...
pub async fn get_3dmigoto_latest_release(game_preset: String) -> Result<UpdateInfo, String> {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("ssmt4-app"),
    );
...
pub async fn install_3dmigoto_update(
    ...
    let target_dir: PathBuf = if let Some(dir) = game_config
        .three_d_migoto
        .get("installDir")
        .and_then(|v| v.as_str())
        .filter(|s| !s.is_empty())
    {
        ...
        if !app_config.cache_dir.is_empty() {
            PathBuf::from(&app_config.cache_dir)
                .join("3Dmigoto")
                .join(&game_name)
        } else {
    ...
    for filename in files_to_copy {
        ...
        if !source_to_use.exists() {
            ...
            if dev_path.exists() {
                ...
                return Err(format!(
                    "Resource file '{}' not found. Checked: {:?}, {:?}, {:?}",
                    filename, src_path, dev_path, dev_path_2
                ));
            }
        }

        fs::copy(&source_to_use, &dest_path).map_err(|e| {
            format!(
                "Failed to copy {}: {}.\nTip: Is the game or 3Dmigoto running? Please close it.",
                filename, e
            )
        })?;
    }
